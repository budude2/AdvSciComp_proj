...
//test AT-filename, rows, cols, b-filename, rows, imsize, iterations, relax
//0    1            2     3     4           5     6       7           8
int main(int argc, char *argv[])
{
    ...
    mat AT = zeros<dmat>(cols_a, rows_a);
    mat b = zeros<dmat>(rows_b, cols_b);

    dmat x;
    x.zeros(AT.n_rows, iterations + 1);

    dmat x0;
    x0.zeros(im_size * im_size, 1);

    AT.load(argv[1]);
    AT.reshape(cols_a, rows_a);

    b.load(argv[4]);

    auto setup_finish = high_resolution_clock::now();

    ART(AT, b, x0, iterations, relax, x);
